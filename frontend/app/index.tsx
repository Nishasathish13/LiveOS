...
function Observation({ workspace }: { workspace: Workspace }) { const latest = workspace.logs[0]; return <View style={styles.observation}><Text style={styles.eyebrow}>AN OBSERVATION</Text><Text style={styles.observationText}>{latest ? `Your latest note was recorded on ${latest.date}.` : "Your first note will give this space a little history."}</Text><Text style={styles.source}>Based only on your saved entries · private</Text></View>; }
...
function Reflect({ workspace, onRefresh }: any) {
  ...
  const domainFact = (d: Domain) => { const anyLog = workspace.logs.find((x: any) => x.domainsTouched?.includes(d.id)); if (!anyLog) return "no entries yet"; const dd = daysAgo(anyLog.date); return dd === 0 ? "logged today" : dd === 1 ? "last logged yesterday" : `last logged ${dd} days ago`; };
  return (<View>
    ...
    {workspace.domains.length === 0 ? <Text style={styles.small}>No domains yet — add some in the Domains tab.</Text> : workspace.domains.map((d: Domain) => <View key={d.id} style={styles.balance}><View style={[styles.dot, { backgroundColor: d.color }]} /><Text style={styles.balanceText}>{d.name} — {domainFact(d)}</Text></View>)}
  </View>);
}
function Domains({ workspace, onRefresh }: any) {
  ...
  return (<View>
    ...
    <Text style={styles.subtitle}>Name the parts of life you want to keep in view.</Text>
    {workspace.domains.map((d: any) => <View key={d.id} style={styles.domainCard}>
      <View style={styles.domainHead}><View style={[styles.dot, { backgroundColor: d.color }]} /><Text style={styles.domainName}>{d.name}</Text><Text style={styles.frequency}>{d.targetFrequency}× / wk</Text></View>
      <Text style={[styles.small, { marginTop: 6 }]}>{d.description || "No description yet"}</Text>
      {d.goals?.length > 0 && <View style={styles.goalList}>{d.goals.map((g: string, i: number) => <View key={i} style={styles.goalRow}><Ionicons name="ellipse" size={6} color={colors.clay} /><Text style={styles.goalText}>{g}</Text></View>)}</View>}
    </View>)}
...
function Companion({ workspace }: any) {
  ...
  const [messages, setMessages] = useState<any[]>(() => (workspace.messages || []).map((m: any) => ({ role: m.role, text: m.text, safety: false })));
...
function Onboarding({ close, onSaved }: any) {
